...
describe('BackupView - Integration & Coverage Tests', () => {
  ...
  it('handles createBackup with data', async () => {
    ...
    mockStores.batch.getBatch.mockResolvedValue({
      id: 1,
      name: 'Batch 1',
      gravity: [{ id: 1, gravity: 1.05, created: '2023-01-01T12:00:00Z' }],
      pressure: [],
      pour: []
    })
  ...
  it('exercises restore process (full flow)', async () => {
    ...
    global.fetch.mockImplementation((url) => {
      ...
      // POST restore
      if (
        url.includes('api/batch/') ||
        url.includes('api/device/') ||
        url.includes('api/gravity/') ||
        url.includes('api/pressure/') ||
        url.includes('api/pour/')
      ) {
