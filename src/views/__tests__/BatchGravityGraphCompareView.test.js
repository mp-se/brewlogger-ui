...
describe('BatchGravityGraphCompareView', () => {
  beforeEach(() => {
    ...
    mockStores.gravity.getGravityListForBatch.mockResolvedValue([
      { id: 1, gravity: 1.05, created: '2023-01-01T10:00:00Z', active: true },
    ...
    const wrapper = mount(BatchGravityGraphCompareView, {
      global: {
        stubs: {
          ...
          BsSelect: {
            template:
              '<select :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)"><option v-for="o in options" :value="o.value">{{o.label}}</option></select>',
